...
class Tau3MuDataset(InMemoryDataset):
                    ...
                    if entry == None: # Don't use an endcap that is empty
                        ...
                        if entry['n_gen_tau']==1: # If signal event, only return hits on tau endcap
                            if ((entry['gen_tau_eta'] * entry[eta]) > 0).sum() == entry['n_mu_hit']: 
                                data_list[i].append((entry, False, endcap))
                            else:
                                data_list[i].append((entry, True, endcap))
                        
            else:
                data_list.append((masked_entry, False))

        # If half model, datalist will be fill of elements (entry, only_eval, endcap), otherwise will just be full of (entry, only_eval)
                            

        if 'half' in self.setting:
            for i in range(2):
                # Now we have cleared out all unused data, find idx_split
                idx_split = Tau3MuDataset.get_idx_split(data_list[i], self.splits, self.pos_neg_ratio)
                out_list = []

                #Now find max's and mins for each set of data
                train_idxs = idx_split['pos_train'] + idx_split['neg_train']
                train_min_maxs = self.compute_mins_maxs(data_list[i], train_idxs)
                
                #Now process entries
                print('[INFO] Processing Entries for', i, '...')
                for entry in tqdm(data_list[i]):
                    if (entry[2] == 1):
                        out_list.append(self._process_one_entry(entry[0], train_min_maxs[1], entry[1], endcap=entry[2]))
                    else:
                        out_list.append(self._process_one_entry(entry[0], train_min_maxs[0], entry[1], endcap=entry[2]))

                data, slices = self.collate(out_list)
                print('[INFO] Saving data.pt...')
                torch.save((data, slices, idx_split), self.processed_paths[i])
        
        else:
            #Same as half detector (look above)
            idx_split = Tau3MuDataset.get_idx_split(data_list, self.splits, self.pos_neg_ratio)
            out_list = []

            #Now find max's and mins
            train_idxs = idx_split['pos_train'] + idx_split['neg_train']
            train_min_max = self.compute_mins_maxs(data_list, train_idxs)
            
            #Now process entries
            print('[INFO] Processing Entries...')
            for entry in tqdm(data_list):
                out_list.append(self._process_one_entry(entry[0], train_min_max, entry[1]))

            data, slices = self.collate(out_list)
            torch.save((data, slices, idx_split), self.processed_paths[0])

    def compute_mins_maxs(self, raw_entries, type_idx):
        print("[INFO] Computing mins/maxs..")
        if 'half' in self.setting:
            ...
            for idx in tqdm(type_idx):
                event = raw_entries[idx][0]
            ...
            return ((neg_mins, neg_maxs), (pos_mins, pos_maxs))

        else:
            elems = [[] for feature in self.feature_names]

            for idx in tqdm(type_idx):
                for j,feature in enumerate(self.feature_names):
                    elems[j].append(raw_entries[idx][0][feature])
                        
            maxs = np.array([np.max(np.concatenate(col)) for col in elems])
            mins = np.array([np.min(np.concatenate(col)) for col in elems])
            return (mins, maxs)

    def _process_one_entry(self, entry, min_max, only_eval, endcap=0):
        
        for i, feature in enumerate(self.feature_names): # Min-max norm
            entry[feature] = (entry[feature] - min_max[0][i]) / (min_max[1][i] - min_max[0][i])
    ...
    def get_idx_split(data, splits, pos2neg):
        ...
        y_dist = np.array([event[0]['y'] for event in data])
        only_eval = np.array([event[1] for event in data])
        ...
        print("[INFO] Successfully split up data!")
